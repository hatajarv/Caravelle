...
# --- Pääsovelluksen osiot: Mittausdata ja huoltohistoria ---
...
# Yhtenäinen malli: Käytetään tätä painotettua mallia myös historiallisten tietojen haussa
...
try:
    # Oletetaan, että huoltohistoria Excel-tiedosto sijaitsee polussa /mnt/data/AYE_599_huoltohistoria.xlsx
...
if not df_huolto.empty:
    # Debug: Näytetään tiedoston ensimmäiset rivit, jotta varmistetaan että sarakkeet ovat oikein
    st.write("Huoltohistorian raakatiedot:")
    st.dataframe(df_huolto.head())
    
    # Oletetaan, että huoltohistoriassa on sarake "Päivämäärä"
    if 'Päivämäärä' not in df_huolto.columns:
        ...
        fp = df['Mittarilukema']
        # Luodaan uusi sarake "Kilometrit" huoltohistoriaan
        ...
        # Piirretään Altair-kuvaaja, jossa näkyy historiallinen mittarilukeman kehitys ja huoltojen merkinnät
